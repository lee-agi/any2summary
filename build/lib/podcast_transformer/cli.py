...
def perform_azure_diarization(
    ...
    # request_extra_body["chunking_strategy"] = chunking_strategy
    ...
    if max_speakers and max_speakers > 0:
        diarization_segments = _limit_speaker_count(diarization_segments, max_speakers)
...
def _load_chunking_strategy() -> Union[str, MutableMapping[str, Any]]:
    """Load chunking strategy from environment or use default."""

    env_value = os.getenv("AZURE_OPENAI_CHUNKING_STRATEGY")
    if env_value:
        ...
        if stripped:
            try:
                ...
                raise RuntimeError(
                    "AZURE_OPENAI_CHUNKING_STRATEGY 解析失败，请提供有效 JSON。"
                ) from exc
            if isinstance(parsed, MutableMapping):
                return parsed
            raise RuntimeError(
                "AZURE_OPENAI_CHUNKING_STRATEGY 必须是 JSON 对象或简单字符串。"
            )
