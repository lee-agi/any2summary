...
import { buildVideoPrompt, buildArticlePrompt, SIMPLE_URL_PROMPT } from "./prompts.js";
 ...
 */
async function checkLocalServerHealth(serverUrl) {
  try {
 ...
 */
async function isLocalServerAvailable(settings) {
  if (!settings.local_server_enabled) {
    return false;
  }
  const health = await checkLocalServerHealth(settings.local_server_url);
...
async function processArticle(url, settings, options) {
  ...
  const article = await fetchArticleContent(url, options.tabId);
